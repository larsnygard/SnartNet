...
mod native {
    ...
    impl FileStorage {
        ...
        pub fn set_item(&self, key: &str, value: &str) -> Result<(), StorageError> {
            use std::io::Write;
            // Replace whole records atomically so a crash cannot truncate the identity or outbox.
            let write = || -> std::io::Result<()> {
                let mut file = tempfile::NamedTempFile::new_in(&self.dir)?;
                file.write_all(value.as_bytes())?;
                file.as_file().sync_all()?;
                file.persist(self.key_path(key)).map_err(|e| e.error)?;
                Ok(())
            };
            write().map_err(|e| StorageError::Backend(format!("write failed for {key}: {e}")))
        }
...
/// Public alias so tests can name a concrete in-memory backend directly.
#[cfg(not(target_arch = "wasm32"))]
