...
impl Post {
    ...
    pub fn to_canonical_json(&self) -> Result<String, String> {
        serde_json::to_string(self).map_err(|e| format!("Failed to serialize post: {}", e))
    }
...
pub fn sign_post(post_json: &str, keypair_json: &str) -> Result<JsValue, JsValue> {
    ...
    let signed_post = SignedPost::create(post, &keypair).map_err(|e| JsValue::from_str(&e))?;
