...
| `SNARTNET_PLATFORM` | `desktop` | Storage and power defaults to apply: `desktop` or `mobile` (`android`/`phone` are accepted) |
| `SNARTNET_STORAGE_REPLICATE` | platform default | Whether this device holds replicas for contacts: `on`/`off` |
| `SNARTNET_STORAGE_QUOTA_MB` | platform default | Replica quota in MiB (desktop 1024, mobile 64) |
| `SNARTNET_STORAGE_LEASE_DAYS` | platform default | How long an accepted replica lease lasts (desktop 30, mobile 7) |
| `SNARTNET_STORAGE_COPIES` | platform default | How many contacts this device asks to hold copies of its own objects |
| `SNARTNET_STORAGE_MIN_FREE_MB` | platform default | Free space on the volume that replicas must leave untouched |
...
Contacts can also hold copies of each other's objects (ADR 0007). A device that hosts replicas accepts a **lease**: a request sealed to that device that names the object, its kind, and its size, signed by the author so the host knows who asked. The host fetches the object itself from the BitTorrent swarm behind the signed DHT pointer, stores the bytes under `root/replicas/`, and returns a **receipt** — its own signed promise to hold the copy until a stated time. That receipt is what moves a message to `replica-stored` in the UI, so the state is evidence rather than a hope. Nothing about the object leaks: a mailbox object is ciphertext the host cannot read, and profiles and feeds are public but signed. What a host stores is bounded locally by a platform default (a desktop volunteers 1 GiB and 30-day leases, a phone volunteers nothing), the user's own settings (`SNARTNET_STORAGE_*`, or the `storage` command), and a per-contact rule that can only narrow. Expired leases are dropped first, then the oldest copies, and the free-space headroom is never eaten, so a host cannot be talked into filling its disk.
...
Since M11 a round of work is bounded (ADR 0008). Publishing, contact lookups, spool draining, and the hand-over of replica records each have a per-round cap, and contact lookups rotate through the list so nobody is starved; the remainder waits for the next round instead of holding everything back. The inbound spool and the in-memory inbox are bounded too, and a full one *refuses* new objects rather than dropping an object that was already acknowledged — the sender keeps it queued and tries again. A round that fails, or that had to turn inbound away, is retried with growing patience (10 seconds doubling up to 10 minutes) and clears as soon as a round completes; the Network panel shows the caps, the queue depths, the refusals, and the current retry delay.
