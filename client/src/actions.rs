...
pub async fn load_startup_async() -> Result<StartupData, String> {
    let storage = FileStorage::open_default().map_err(|e| e.to_string())?;
    load_startup(&storage)
}

pub fn load_startup(storage: &FileStorage) -> Result<StartupData, String> {
...
pub async fn create_profile_async(
    username: String,
...
pub async fn add_contact_async(fingerprint: String, alias: String) -> Result<Contact, String> {
...
pub async fn create_message_async(
...
pub async fn import_invite_async(code: String) -> Result<Contact, String> {
